var Uso, config, createPost, createScript, createTopic, downloadSource, fs, github, http, modifyScript, path, qs, router, saveScripts, scripts, scripts_path, unwatchScript, url_parser, uso;
  ...
  return downloadSource(commit.url, file, function(source) {
    return uso.login(function(error) {
      ...
      return uso.updateScript(script.id, source, function(error, response) {
        ...
        return script.topic ? createPost(file, commit) : createTopic(file, commit);
      });
    });
  });
};
...
router.post('/' + config.hook_path).module('post').bind(function(request, response, next) {
  var _i, _j, _k, _l, _len, _len2, _len3, _len4, _len5, _m, _ref, _ref2, _ref3, _ref4, _result, body, changed_files, changes, commit, file, found, hook_owner, owner, task;
  ...
  for (file in changes) {
    ...
    _result.push((function() {
      switch (task.state) {
        case 'changed':
          return scripts[file] ? modifyScript(file, task) : createScript(file, task);
        case 'removed':

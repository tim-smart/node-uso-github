...
var SCRIPTS_PATH, config, createPost, createScript, createTopic, downloadSource, error, fs, https, loadScripts, m, modifyScript, muso, path, qs, router, saveScripts, scripts, unwatchScript, url_parser, uso;
...
uso = muso.createClient({
  email: config.uso.username,
  password: config.uso.password
});
...
createTopic = function(file, commit) {
  ...
  return uso.createTopic({
    id: script.id,
    title: 'Change Log',
    body: body
  }, function(error, topic_id) {
    if (error) {
...
createPost = function(file, info) {
  var body, commit, commits, compare, script, _i, _len, _ref;
...
router.post(config.hook_route).bind(m.post()).bind(function(request, response, next) {
  var body, changed_files, changes, commit, file, found, hook_owner, owner, task, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _ref, _ref1, _ref2, _ref3, _results;
  ...
  for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
    ...
    for (_k = 0, _len2 = changed_files.length; _k < _len2; _k++) {
  ...
  for (file in changes) {
    ...
    switch (task.state) {
      case 'changed':
        if (scripts[file]) {
          _results.push(modifyScript(file, task));
        } else {
          _results.push(createScript(file, task));
        }
        break;
      case 'removed':
        _results.push(unwatchScript(file, task));
        break;
      default:

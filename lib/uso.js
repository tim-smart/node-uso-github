...
Uso = (function() {
  ...
  Uso.prototype.login = function(done) {
    var self,
      _this = this;
    self = this;
    return this.get('/login', function(error, response) {
      var body;
      if (error) {
        return done(error);
      }
      body = qs.stringify({
        ...
        password: _this.pass,
        remember_me: '0',
        authenticity_token: _this.getAuthToken(response.body)
      });
      return usoRequest(_this, {
        method: 'POST',
        uri: '/sessions',
        body: body,
        headers: {
  ...
  Uso.prototype.createScript = function(source, done) {
    var _this = this;
    return this.get('/scripts/new?form=true', function(error, response) {
      if (error) {
        return done(error);
      }
      return _this.post({
  ...
  Uso.prototype.updateScript = function(id, source, done) {
    var _this = this;
    return this.get("/scripts/edit_src/" + id, function(error, response) {
      if (error) {
        return done(error);
      }
      return _this.post({
  ...
  Uso.prototype.deleteScript = function(id, done) {
    var _this = this;
    return this.get("/scripts/show/" + id, function(error, response) {
      if (error) {
        return done(error);
      }
      return _this.post({
        body: {
          authenticity_token: _this.getAuthToken(response.body, true)
        },
  ...
  Uso.prototype.createTopic = function(type, id, title, body, done) {
    var _this = this;
    return this.get("/topics/new?" + (type.toLowerCase()) + "_id=" + id, function(error, response) {
      if (error) {
        return done(error);
      }
      return _this.post({
  ...
  Uso.prototype.createPost = function(topic_id, body, done) {
    var _this = this;
    return this.get("/topics/" + topic_id, function(error, response) {
      if (error) {
        return done(error);
      }
      return _this.post({
